...
* Solution: Reduce problem to minimum disjoint path cover of a DAG, which is
*           solved by matching.
******************************************************************************/
  ...
  while (n--)
  {
    ...
    for (i=0;i<p;i++)
    {
      ...
      for (j=0;j<i;j++)
      {
        ...
        if (arrtid<tid) reachs[j][nreach[j]++]=i; // State i is reachable from state j
      }
    }
